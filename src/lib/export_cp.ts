...
export function exportar(acta: any) {
  ...
  pdf.setFontSize(10);
  const splitPlanteamiento = pdf.splitTextToSize(acta.planteamientos, 180);
  pdf.text(splitPlanteamiento, 14, yPos + 5);
  yPos += 20 + splitPlanteamiento.length * 5;
  ...
  pdf.setFontSize(10);
  yPos += 5;
  const splitValoracion = pdf.splitTextToSize(acta.valoracion, 180);
  pdf.text(splitValoracion, 14, yPos + 5);
  yPos += 20 + splitValoracion.length * 5;
