<template>
    <div class="min-h-screen bg-gray-100 py-12 px-4 sm:px-6 lg:px-8">
      <div class="max-w-7xl mx-auto bg-white rounded-xl shadow-md overflow-hidden">
        <div class="p-8">
          <h1 class="text-3xl font-bold text-gray-900 mb-6">{{ acta.name }} {{acta.id}}</h1>
          ...
          <div class="space-y-6">
            ...
            <section>
              ...
              <div class="grid grid-cols-4 gap-4">
                <span class="font-medium text-gray-600">Núcleo:</span> {{ acta.core?.name }}
                <span class="font-medium text-gray-600">Secretario General:</span> {{ acta.secretarioGeneral }}
                <span class="font-medium text-gray-600">Fecha:</span> {{ acta.fecha }}
                <span class="font-medium text-gray-600">Lugar:</span> {{ acta.lugar }}
                <span class="font-medium text-gray-600">Hora:</span> {{ acta.hora }}
              </div>
            </section>
              ...
              <div class="overflow-x-auto">
                <table class="min-w-full divide-y divide-gray-200">
                  ...
                  <tbody class="bg-white divide-y divide-gray-200">
                    <tr v-for="miembro in acta.militante" :key="miembro.nro" class="hover:bg-gray-50">
                      <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ miembro.id }}</td>
                      <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{{ miembro.firstname }}</td>
                      ...
                      <td class="px-6 py-4 text-center whitespace-nowrap text-sm text-gray-900">
                        <span v-if="miembro.causa === ''">{{ miembro.causa || '-' }}</span>
                      </td>
                    </tr>
                  </tbody>
                </table>
            ...
            <section>
              ...
              <ol class="list-decimal list-inside space-y-1">
                <li v-for="(item, index) in acta.order" :key="index" class="text-md text-gray-600">
                  {{ item }}
                </li>
              </ol>
            </section>
            ...
            <section>
              <h2 class="text-xl font-semibold text-gray-800 mb-2">1. Chequeo de acuerdos</h2>
            ...
            <section>
              <h2 class="text-xl font-semibold text-gray-800 mb-2">2. Orientaciones del Organismo Superior</h2>
              <div class="mb-4">
            ...
            <section>
              <h2 class="text-xl font-semibold text-gray-800 mb-2">3. Análisis</h2>
              <div class="mb-4">
            ...
            <!-- Acuerdos -->
            <section>
              <h2 class="text-xl font-semibold text-gray-800 mb-2">4. Acuerdos</h2>
              <ul class="space-y-2">
                <li v-for="acuerdo in acta.agreements" :key="index" class="text-md text-gray-600">
            ...
            <section>
              <h2 class="text-xl font-semibold text-gray-800 mb-2">5. Salidas al extranjero</h2>
              <ul class="space-y-2">
                <li v-for="salida in acta.extranjero" :key="index" class="text-md text-gray-600">
            ...
            <section class="border-t border-gray-300">
              <h2 class="text-xl font-semibold text-gray-800 my-4">Próximas fechas</h2>
              <div class="grid grid-cols-2 gap-4 text-md text-gray-600">

                <span class="font-medium">Fecha de la Próxima Reunión Ordinaria:</span> {{ acta.fechaProx }}
                <span class="font-medium">Fecha de la Preparación de la Próxima Reunión Ordinaria:</span> {{ acta.fechaPrep }}
                <span class="font-medium">Fecha del Próximo Círculo de Estudios Políticos:</span> {{ acta.fechaCP }}
              </div>
            </section>
          </div>
        </div>
  ...
  async function getActa() {
    ...
    try{
      acta.value = await services.getMinute(id)
      console.log(acta)
    }
    catch (e) {
      console.error(e)
