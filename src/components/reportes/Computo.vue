  ...
  <div class="min-h-screen p-8 dark:bg-zinc-800">
    ...
    <div class="bg-white rounded shadow-md p-4 container mx-auto">
      <div class="p-8 space-y-6">
        ...
        <!--Table-->
        <div class="flex rounded-lg border bg-white shadow-md">
            ...
            <tbody class="bg-white divide-y divide-gray-200">
              <tr v-for="item in filteredData" :key="item.nucleo.id">
                ...
                <td
                  ...
                  :class="{ 'text-red-600 font-semibold': item.pto < 3 }"
                >
                  {{ item.pto }}
                </td>
                <td
                ...
                >
                  {{ item.totalDeAcuerdos}}
                </td>
                <td
                  class="px-6 py-4 whitespace-nowrap text-sm text-center text-gray-500"
                >
                  {{ item.participacionOrgSup }}
                </td>
                <td
                  class="px-6 py-4 whitespace-nowrap text-sm text-center text-gray-500"
                >
                ...
                <td
                  class="px-6 py-4 whitespace-nowrap text-sm text-center"
                  :class="{ 'text-red-600 font-semibold': item.cp === 0 }"
                >
                  {{ item.cp }}
                </td>
                <td
...
const filteredData: Indicadores = computed(() => {
  return data.value.filter((item) => {
    if (selectedNucleo.value && item.nucleo.name !== selectedNucleo.value)
      return false;
    return true;
  });
});
